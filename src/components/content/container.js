      ...
      return (
        <div className="flex items-center gap-4">
          ...
          <a href={props.git} target="blank">
            <FaGithub size="35" className="hover:text-purple-mid duration-300"/>
          </a>
        </div>
      );
    } else if (links === "web") {
  ...
  return (
    <div className="overflow-hidden p-2 flex flex-col items-center">
      <div className="h-[100%] flex ">
        <Image src={imagePath} width={1000} height={1000} alt={props.alt} className="rounded-xl"/>
      </div>
      ...
      <div className="flex flex-col items-center">
        ...
        <div className="capitalize flex gap-2 justify-center my-4 text-sm">
          {tech.map((item) => (
            <p className="font-medium px-4 py-1 text-slate-700 bg-purple-mid/10 rounded" key={item}>
              {item}
            </p>
          ))}
        </div>
